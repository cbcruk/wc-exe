...
- 타이밍(이 Linux 샌드박스): 번들+minify 버스트 ~0.3–0.43s, esbuild-wasm 초기화 ~0.35s 포함 총 ~0.75–0.95s. 같은 머신의 네이티브 `vite build`는 157ms 자체 보고 → **네이티브 대비 2~3× 느림**. ⚠️ `bench/README.md`의 수치는 macOS에서 잰 것이라 **직접 비교 금지**(WebContainer는 이 샌드박스에서 부팅 불가).
