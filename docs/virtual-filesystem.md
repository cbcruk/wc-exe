...
**둘 다 불가능한 환경이 이 프로젝트의 전제다.** 대상은 **극단적 폐쇄망**이고, 거기서는 RAM 디스크를 마운트할 관리자 권한도 백신 정책을 바꿀 권한도 없다. 브라우저는 그 통제 아래에서 **이미 승인되어 돌아가는 프로세스**이고, 그 안의 메모리는 정책이 손대지 않는 유일한 공간이다. 그래서 브라우저 경로다.

**이 조건은 다른 것도 함께 정한다.** 폐쇄망이면 npm 레지스트리도 내부 미러여야 하고(양쪽 경로에 똑같이 필요하다), **부팅에 외부 호스트를 요구하는 런타임은 애초에 못 쓴다.** 그런데 지금 프로덕션 경로인 WebContainer가 정확히 그렇다 — 확정 7이 기록한 대로, 이 저장소의 리눅스 샌드박스에서 **런타임 호스트가 차단되자 부팅 자체가 되지 않았다.**

→ 즉 이 전제는 브라우저 경로를 정당화하는 동시에 **WebContainer 의존을 벗어날 이유를 이론에서 요구사항으로 바꾼다.** 다만 실제 대상 환경에서 WebContainer가 뜨는지는 아직 확인되지 않았다(§10 미결 11). 뜬다면 허용 목록에 그 호스트들이 들어 있다는 뜻이고, 안 뜬다면 **현행 프로덕션 경로가 그 환경에서 동작하지 않는다**는 뜻이라 우선순위가 통째로 바뀐다.
...
- **인터셉트는 PoC로 유지한다 — 단, 미결 11의 답을 기다리는 상태다.** 성능·개방성(CDN 불필요)은 매력적이지만 범용성이 아직 약속을 못 지킨다. **승격한다면 `--vfs=memfs` 변형이다**(확정 6). 승격 조건으로 적어뒀던 "WebContainer 독점 의존이 실제로 발목을 잡으면"이 §1의 경계 조건 확정으로 **가정이 아니라 확인 대기 항목**이 됐다 — 대상 폐쇄망에서 WebContainer가 부팅되지 않는다면 인터셉트는 승격 후보가 아니라 유일한 경로다.
...
11. **대상 환경에서 WebContainer가 부팅되는가** — 미결 10의 답이 바로 만들어낸 질문이고, **지금 가장 결정적인 미측정이다.** 폐쇄망이면 부팅에 외부 호스트를 요구하는 런타임은 못 쓰는데, WebContainer가 정확히 그렇다(확정 7). 뜬다면 허용 목록에 StackBlitz 호스트가 들어 있다는 뜻이다. **안 뜬다면 현행 프로덕션 경로가 대상 환경에서 동작하지 않는다는 뜻**이고, 그러면 미결 8(install 없는 인터셉트)이 "있으면 좋은 것"에서 **유일하게 남는 경로**로 바뀐다. 측정 방법은 간단하다 — 대상 망에서 `wc-exe build`를 한 번 돌려본다. **npm 트리 해석을 재현해야 하는 부분이 비용의 본체**라는 «하이브리드 착지점»의 경고가 그대로 적용된다.
