...
**그런데 비용이 사라진 게 아니라 옮겨갔다.** 템플릿은 클로저가 작지만(0~23개, 53~80MB 중 16MB 이하), 현실적인 앱은 뒤집힌다 — 직접 `npm install`한 것은 런타임 의존성이므로 `vue-app`의 클로저가 135개 중 118개, 79.2MB(필터 후 **38.4MB**)다. 그리고 그 숫자는 **그걸 미리 다 옮겨야 하는 모드**에 떨어진다. `--vfs=memfs`는 lazy fetch가 불가능하고, 15.2MB가 이미 React 빌드에서 ~280ms를 문다. 38MB면 2.5배다.

**즉 install 없는 인터셉트는 정확성 쪽에선 열려 있고, 문제를 전송량으로 옮긴다** — `--vfs=memfs`가 이미 부딪힌 그 벽이 더 커진 형태다. 다만 한 가지 모양은 그 문제를 피한다: **풀어놓은 트리가 아니라 타르볼을 캐싱하는 것.** §5의 cacache가 이미 그렇게 하고 있고, 여기서도 옳은 모양이다 — 백신 비용은 **파일 개수당** 발생하는데, 40MB의 타르볼은 수백 개 파일이고 풀어놓은 트리는 13,340개다.
...
8. **install 없는 인터셉트** — 인터셉트가 wc-exe의 실제 문제를 풀려면 반드시 필요하다(§9). **①단계 완료: 10개 프로젝트 실측에서 런타임 클로저의 lifecycle 훅은 0개다**(`bench/install-shape.mjs`, 아래 «훅은 비어 있고, 비용은 옮겨갔다»). 남은 것: ② 호스트가 lockfile → 타르볼 → 메모리 → 볼륨을 잇는다. **npm 트리 해석을 재현해야 하는 부분이 비용의 본체**라는 «하이브리드 착지점»의 경고가 그대로 적용된다.
