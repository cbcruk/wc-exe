...
### 왜 호스트 쪽 메모리 파일시스템은 아닌가

문제를 "백신이 **디스크에 쓰인 파일**을 스캔한다"로 정의하면, 브라우저 메모리 말고도 같은 성질을 주는 것들이 있다 — **tmpfs / RAM 디스크**에 `node_modules`를 두고 네이티브 node로 설치하기, 혹은 **백신 제외 규칙**을 거는 것. 둘 다 이 프로젝트 전체보다 훨씬 싸다.

**답은 통제된 장비에서는 둘 다 권한이 없다는 것이다.** RAM 디스크를 마운트하려면 관리자 권한이 필요하고, 백신 제외 규칙은 정책이 잠근다. 브라우저는 그 통제 아래에서 **이미 승인되어 돌아가는 프로세스**이고, 그 안의 메모리는 정책이 손대지 않는 공간이다. 그래서 브라우저 경로다.

**다만 정확히 적어둔다: 이 문서를 쓰는 시점에 그런 환경이 현재 조건은 아니다.** 극단적 폐쇄망은 과거 작업에서 온 **동기가 된 시나리오**이고, 설계를 어느 쪽으로 기울일지는 정하지만 **지금 재볼 수 있는 제약은 아니다.** 이 구분이 중요한 이유는, 한때 "가장 결정적인 미측정"으로 적혔던 미결 11이 실은 **측정 가능한 항목이 아니었기** 때문이다(§10).

**시나리오로서는 여전히 무언가를 정한다.** 폐쇄망이면 npm 레지스트리도 내부 미러여야 하고(양쪽 경로에 똑같이 필요하다), **부팅에 외부 호스트를 요구하는 런타임은 못 쓴다.** WebContainer가 정확히 그렇다 — 확정 7이 기록한 대로, 이 저장소의 리눅스 샌드박스에서 **런타임 호스트가 차단되자 부팅 자체가 되지 않았다.** 그건 가상이 아니라 관측이다. 그러니 "WebContainer 의존을 벗어나기"는 **요구사항은 아니고, 근거 있는 선호**다.
...
> ⚠️ **B의 0.30s는 재측정이 필요하다.** 이 표는 OPFS 스냅샷 복원이 **실행 권한을 보존하지 않던 시절**에 잰 것이다. 복원된 `node_modules/.bin`이 실행 불가라 `npm run build`가 `spawn vite EACCES`를 찍고도 **종료 코드 0으로 끝났고**, 그래서 캐시가 붙은 빌드가 조용히 산출물을 하나도 만들지 않았다 — `docs/persistent-runner.md` §12.1이 이걸 발견하고 고쳤다(복원 직후 `chmod -R +x`). **install 단계를 스킵한다는 사실 자체는 유효하지만, 0.30s는 그 chmod 비용을 포함하지 않고 뒤따르는 빌드가 실패하는 상태에서 나온 값**이다. 이 저장소의 샌드박스에서는 WebContainer가 부팅되지 않아(확정 7) 여기서 다시 잴 수 없다. **로컬에서 `node bench/cache-scenarios.mjs`를 다시 돌려야 하는 항목이다**(미결 9).
...
두 갈래가 같은 답으로 수렴한다. **그리고 지금은 그 질문을 재볼 대상 환경조차 없다**(미결 11) — 폐쇄망은 동기가 된 시나리오이지 현재 조건이 아니다. 즉 착수 근거는 "언젠가 필요할지도"이고, 그건 2년을 걸 근거가 못 된다.
...
**다음에 결판낼 것**: ① 한 머신에서 WebContainer `npm run build`와 동일 조건 비교(미결 1) — 한동안 여기 "지금 그대로 재면 빌드 단계만의 비교"라는 단서가 붙어 있었는데, `--deps=registry`로 미결 8이 닫히면서 **해소됐다** ② 플러그인 호환성·sourcemap·multi-page. 자세한 내용은 `poc/vite-build-intercept/README.md`.
...
둘로 갈랐다. **제네릭**(항상): HTML이 참조하는 JS/CSS 애셋이 실재하는가, TS/JSX 문법이 남았는가, **방출된 스타일시트가 JS 안에 있는가**(CSS 자산 가운데에서 40자를 떼어 JS에 없는지 확인 — 옛 `#213547` 검사의 내용 무관 버전), 애셋 URL이 디스크에 있는가, 런타임에 에러·실패 요청이 없고 **뭔가 렌더됐고**(엔트리가 실행 안 되면 HTML이 실어보낸 빈 div가 그대로 남는다) 스타일시트가 규칙을 하나라도 냈고 모든 `<img>`가 `naturalWidth > 0`인가. **옵트인**(`wc-exe-verify.json`): 앱이 무슨 말을 하는지 알아야 하는 것들 — `cssMarker`·`jsMarker`·`renderedText`·`counter`·`lazy`·`mustContain`/`mustNotContain`(기존 resolution 기대치를 흡수).

**그리고 처음으로 임의 프로젝트를 쟀다.** `npm create vite@latest` 템플릿 7개, 기대치 파일 없음:
...
| preact-ts  | ✅   | ✅   | —                                                              |
| lit-ts     | ✅   | ✅   | —                                                              |
| vue-ts     | ❌   | —    | `.vue`에 Vue 컴파일러 필요                                     |
| svelte-ts  | ❌   | —    | `.svelte`에 Svelte 컴파일러 필요                               |
| solid-ts   | ✅   | ❌   | Solid의 JSX 변환 — 빌드는 되고 런타임에 `Unexpected token '<'` |

**7개 중 4개, 그리고 실패 3개의 원인이 전부 같다** — 이 파이프라인이 돌리지 않는 프레임워크 플러그인. 미결 2가 예측이 아니라 숫자를 갖게 됐다. solid-ts 행이 교훈적이다 — **깨끗하게 빌드되고 브라우저에서 깨진다.** 런타임 검사가 선택이 아닌 이유다.

**그 과정에서 하네스 버그 두 개가 즉시 드러났다.** ① 런타임 서버가 `.html`·`.js`·`.css`만 타입을 붙여서 **SVG가 `application/octet-stream`으로 나갔다** — 브라우저는 PNG는 스니핑해 렌더하지만 SVG는 의도적으로 안 한다. "image did not load" 3건이 빌드가 아니라 하네스였다. ② **HTML이 직접 링크한 스타일시트를 버리고 있었다** — 모듈 그래프가 닿은 CSS만 모았으므로, `<link rel="stylesheet" href="./src/index.css">`를 쓰는 lit 템플릿은 `dist/`에 없는 파일을 가리키는 HTML을 냈다. 둘 다 **저장소 안에서는 보이지 않았다. 그렇게 하는 픽스처가 없었으니까.**
...
위 거래는 수제 리졸버가 실제로 틀릴 때만 값을 한다. 그래서 픽스처를 하나 추가했다 — 로컬 패키지 4개가 각각 **`node` 파일과 `browser` 파일에 다른 마커**를 담고, `wc-exe-verify.json`이 번들에 무엇이 있어야 하고 무엇이 없어야 하는지를 선언한다. 잘못 고르면 **조용히 성공**하기 때문이다: 빌드는 되고 앱은 뜨는데 잘못된 파일이 실려 나간다.
...
1. **저장 계층은 우리가 소유한다 — 완료.** OPFS 스냅샷 캐시로 install이 11.66s → **0.30s**(캐시 히트 시 전면 스킵). ⚠️ **이 0.30s는 실행 권한 버그가 고쳐지기 전에 잰 값이라 재측정 대상이다**(§5의 경고, 미결 9). lockfile이 바뀌어도 타르볼 cacache가 살아남아 11.35s → **5.74s**(1.98×). 두 캐시 축(lockfile별 스냅샷 / 전역 누적 타르볼)의 분리가 부분 무효화를 가능하게 한 핵심이다(§5).
...
**그리고 경계 이전에 구멍이 하나 있다.** 위 두 경계는 "빌드가 무엇을 못 하나"인데, 그보다 앞서 **인터셉트는 `npm install`을 하지 않는다.** 디스크에 이미 있는 `node_modules`를 읽는다 — 즉 wc-exe가 없애려던 그 작업을 사용자가 먼저 해야 한다. 대체하는 조각도 가장 작다(부팅 5.4s + install 0.30s + 빌드 1.6s 중 빌드만 — 그리고 부팅은 데몬이 이미 상각한다, 미결 0). 다만 실측해보면 그 벽이 낮을 수 있다는 신호가 있다(§9 《install 없는 인터셉트》): React 픽스처 71개 패키지 중 인터셉트 빌드에 걸리는 lifecycle 훅은 **0개**이고, 필요한 런타임 클로저는 **5개·5MB**다.
...
- **인터셉트는 PoC로 유지한다 — 판단을 막는 것은 미결 2다.** (한동안 "미결 11의 답을 기다리는 상태"라고 적혀 있었는데, 같은 문단이 끝에서 스스로 반박하고 있었다. 미결 11은 잴 대상이 없어 닫힌 항목이므로 무언가를 기다리게 만들 수 없다.) 성능·개방성(CDN 불필요)은 매력적이지만 범용성이 아직 약속을 못 지킨다. **승격한다면 `--vfs=memfs` 변형이다**(확정 6). 승격 조건으로 적어뒀던 "WebContainer 독점 의존이 실제로 발목을 잡으면"은 **여전히 가정이다**(미결 11 참조 — 잴 대상 환경이 없다). 다만 그동안 인터셉트 쪽이 혼자 많이 나아갔다: `--deps=registry`로 `npm install` 없이 돌고, 스톡 템플릿 5/7을 빌드한다. 승격 판단은 이제 "WebContainer가 막히나"가 아니라 **"나머지 2/7과 플러그인 생태계를 감당할 것인가"** 로 옮겨간다.
...
- **1. 한 머신에서 WebContainer `npm run build` vs 인터셉트 비교.** 이 저장소의 벤치 수치는 macOS, PoC 수치는 Linux 컨테이너에서 잰 것이라 직접 비교가 불가하고, WebContainer는 이 샌드박스에서 부팅되지 않는다.

  **한동안 여기 붙어 있던 "사과와 오렌지" 단서는 해소됐다.** 그 단서는 "미결 8이 남아 있는 한 인터셉트는 `npm install`을 하지 않으므로 빌드 단계만 비교하게 된다"였는데, 미결 8이 `--deps=registry`로 닫혔다. 이제 두 경로가 같은 일을 한다. 남은 조건은 **WebContainer가 뜨는 기계 하나**뿐이고, 그 위에서 데몬 warm(부팅 상각됨)과 인터셉트 warm을 나란히 재면 된다.

  **답하면 달라지는 것**: 인터셉트를 승격할 이유가 있는지 자체. 불리하게 나오면 미결 5는 그을 이유가 없어지고, 인터셉트는 "WebContainer가 막혔을 때의 대비책"이라는 좁은 지위로 내려간다.

- **2. 플러그인 호환성** — 위에서 말한 경계. 무엇을 얼마나 지원할지가 곧 범위 결정이다. **실측: `npm create vite` 템플릿 7개 중 5개 통과**(§9). Vue SFC를 붙여 4→5가 됐다.

  **답하면 달라지는 것**: 이게 승격 여부를 쥔 항목이다 — 권고가 명시적으로 "판단을 막는 것은 미결 2"라고 적어뒀다. 다른 미결과 달리 실제 기계도, 새 계측기도 필요 없다. 막는 것은 브라우저용으로 배포되지 않는 컴파일러를 어떻게 실행할 것인가 하나뿐이다.

  남은 둘(svelte·solid)에 대해 이 항목은 오랫동안 **문제가 둘**이라고 적어뒀다 — 컴파일러가 브라우저용으로 배포되지 않는다는 것, 그리고 컴파일러가 devDependency라는 것. **후자는 이미 대부분 풀려 있었다.** `--deps=registry`의 `readLockfile`이 `includeDev` 옵션을 갖고 있다(`registry.mjs:107`). 다만 **부르는 곳이 하나도 없다** — 플래그 하나 거리이지 미지의 문제가 아니다. 그러니 이 항목의 진짜 미지수는 **브라우저용으로 배포되지 않는 컴파일러를 어떻게 실행할 것인가** 하나로 좁혀진다.

  저비용 수단 하나가 여기 붙어 있다: **OXC로 `vite.config.ts`를 정적 추출**하는 것(§9 vrowzer). 포크 없이 설정을 읽고, 못 다루는 것은 `unsupported[]`로 신고한다. (한때 미결 7이었는데, 이건 모르는 것이 아니라 **해볼 만한 수단**이라 항목에서 내렸다 — 아래 «미결이 아니었던 것».)
...
- **3. 미검증 생태계 형태** — **상당 부분 해소.** `browser` 필드 remap·`imports` 필드·깊은 `exports` 와일드카드는 `sample-exports-app`으로, **애셋 import와 `public/`는 `sample-asset-app`으로** 검증했다(둘 다 네이티브 vite 기준 대조). 양쪽 다 **`--vfs=memfs`만 통과한다**(§9). 남은 것: CSS `url()` 참조, `?url`/`?raw` 쿼리, `new URL(…, import.meta.url)`, worker/wasm import, sourcemap, multi-page.
  ...
  **언제 답할 것인가: 승격이 실제로 논의될 때.** 한때 여기에 "살아있는 것 중 유일하게 이 환경에서 닫을 수 있다"고 적어두고 다음 작업으로 올려놨었는데, **그건 이유가 아니었다.** 지금 이걸 닫아도 달라지는 결정이 없다 — 승격 판단은 미결 2(브라우저용으로 배포되지 않는 컴파일러)와 미결 5(`Builder` 이음새, 미결 1에 걸려 있다)가 쥐고 있고, 권고는 여전히 "인터셉트는 PoC로 유지한다"이다. 제품 경로는 프로젝트의 진짜 vite를 돌리므로 이 형태들이 아예 닿지 않는다(산출물 왕복 `uploadDist` → `writeDistFile`이 바이너리 안전한지는 따로 확인했다 — 인코딩 단계가 없어 깨끗하다).

  답하는 방법 자체는 준비돼 있다: PoC는 WebContainer가 필요 없어 이 샌드박스에서도 완주하고(`CHROME_PATH`만 있으면 된다 — 확인함), 방법론 ②대로 형태마다 그것만 보는 픽스처를 쓰고 네이티브 `vite build`와 대조하면 된다. 승격이 실제 선택지가 되는 날 그대로 하면 된다.

  **다만 이 영역의 진짜 위험은 커버리지가 아니라 부패다.** PoC는 1000줄이 넘는데 `vitest` 대상이 아니고, 사람이 손으로 돌릴 때만 실행된다. §21이 보여준 대로 그런 코드는 조용히 죽는다 — `bridge.mjs`는 PR 두 개 동안, 벤치 셋은 그보다 오래 죽어 있었고 아무도 몰랐다. 여기서 뭔가 한다면 **"CSS `url()`을 처리하나"가 아니라 "PoC가 아예 안 돌게 되면 알아차리나"** 쪽이다. 픽스처와 검증 하네스는 이미 있고, 걸림돌은 Chrome이 필요해 `pnpm test`에 그대로 못 넣는다는 것뿐이다.

- **5. `Builder` 이음새** — 인터셉트를 두 번째 백엔드로 올리려면 `Runtime` 위에 "프로젝트 → `dist/`" 층이 필요하다(위 «이음새의 고도»). **미결 1의 측정이 유리하게 나온 뒤에** 긋는다. 순서를 바꾸면, 쓸지도 모르는 것을 위해 층부터 얹게 된다.
...
- **0. 부팅 상주화(persistent runner)** — **완료.** 설계·구현 모두 끝났고(`docs/persistent-runner.md` Phase 0~2, `src/daemon/`), `wc-exe build --daemon`으로 쓴다. 실측(macOS, sample-vite-app): 일회성 `--cache` warm **4.2~5.1s** → 데몬 warm **2.0~2.2s**, 약 2배. 남은 단서 둘: 절약분의 상한이 부팅 시간이라 **빌드 자체가 큰 프로젝트일수록 상대 이득이 줄고**, 기본값은 여전히 꺼져 있다.

- **6. fs-proxy memfs 이식 실험** — **완료(`--vfs=memfs`).** 동작하고 출력이 바이트 동일하며, 지연 VFS도 **되찾았다**(동기 XHR fault-in) — 대가가 1.8×에서 약 9%로 내려갔고 규모가 큰 앱에선 eager 대비 4×다. 미결 3의 해당 형태들은 이 모드에서만 올바로 해석된다. vrowzer의 10MB 페이로드 패치는 **필요 없었다**. §9 참조.
...
- **10. 왜 호스트 쪽 메모리 파일시스템은 아닌가** — **답이 나왔고 §1에 적었다.** 극단적 폐쇄망이라 RAM 디스크 마운트도 백신 정책 변경도 권한이 없다. 브라우저는 이미 승인되어 돌아가는 프로세스이고 그 메모리가 정책이 손대지 않는 유일한 공간이다.

- **11. 대상 환경에서 WebContainer가 부팅되는가** — **잘못 세운 항목이었다. 측정할 대상 환경이 없다.** 폐쇄망은 과거 작업에서 온 동기이지 현재 조건이 아니다(§1). 한때 이 항목을 "가장 결정적인 미측정"으로 적었는데, 재볼 수 없는 것을 그렇게 부르면 **다른 항목들의 우선순위가 없는 근거로 밀린다.** 그런 환경을 다시 만나면 그때 `wc-exe build` 한 번으로 답이 나오고, 결과에 따라 미결 8이 선택지에서 유일한 경로로 바뀐다 — 그때까지는 **선호이지 요구사항이 아니다.**

  (남겨두는 이유는 방법론 ③과 같다: 측정 불가한 항목을 결정 근거로 쓰려 했던 것 자체가 기록할 값어치가 있다.)

#### 미결이 아니었던 것 (2026-08 정리에서 내림)

목록이 «모르는 것»과 «해볼 만한 것»을 섞으면 우선순위를 매기는 기능을 잃는다. 미결 11이 당한 것과 같은 실패이고(방법론 ④), 이번엔 반대 방향이다 — 재야 할 것이 아니라 **정하면 되는 것**이 섞여 있었다.

- **4. 캐시 고도화** — 명시된 문제(cacache blob 무한 증가)는 **상한으로 이미 막았다**(§5). 남은 것은 lockfile diff 기반 부분 무효화(burrow `src/npm`의 stale-lock retry가 원형)인데, **이득이 미측정이고 막고 있는 것이 없다.** 두 캐시 축의 분리만으로 lockfile 변경 MISS가 이미 1.98× 싸다(확정 1). 하고 싶으면 하는 것이지, 답을 기다리는 항목이 아니다.

- **7. OXC `vite.config.ts` 정적 추출** — 미지수가 없다. **미결 2를 좁히는 수단**이므로 그 항목 안으로 옮겼다.

### 문서 정합성에 대하여

이 노트는 실측을 기록하는 문서라, **수치가 낡으면 결론이 조용히 틀린다.** 2026-08 점검에서 실제로 나온 것들:

- **미결 0이 이미 풀려 있었다.** 데몬이 제품에 있고(`src/daemon/`) `docs/persistent-runner.md`가 완료를 적어뒀는데, 이 문서는 여전히 "부팅 5.4s가 지배적 비용"이라고 말하고 있었다. 두 문서가 같은 저장소에 있으면서 서로를 안 읽었다.
- **확정 1의 헤드라인 수치가 깨진 상태를 잰 값이었다.** 그것도 `persistent-runner.md` §12.1이 **이미 적어둔** 사실이다.
- **§1이 가장 값싼 대안(tmpfs·제외 규칙)을 한 번도 검토하지 않았다.**

2026-08 두 번째 점검(미결 목록 정리)에서 더 나온 것들:

- **미결 1의 전제가 미결 8이 닫히면서 무효가 됐는데 본문은 그대로였다.** "사과와 오렌지를 비교하게 된다"는 단서가, 그 단서를 만든 항목이 완료된 뒤로도 두 곳(§9 요약, 미결 1)에 남아 있었다.
- **미결 2가 실제보다 크게 적혀 있었다.** "컴파일러가 devDependency"라는 절반은 `readLockfile`의 `includeDev`로 이미 열려 있었다 — 부르는 곳이 없을 뿐이다. 코드가 앞서갔는데 목록이 따라오지 않았다.
- **권고 절이 자기 문단 안에서 모순돼 있었다.** "미결 11의 답을 기다린다"로 시작해서 "이제 질문은 11이 아니라 나머지 2/7"로 끝난다.
- **계측기가 조용히 죽어 있었다.** puppeteer 제거의 부수 효과로 벤치·통합 스크립트 셋이 import에서 죽고, PoC는 락파일에 없는 패키지를 스토어 잔재로 해석하고 있었다. **`pnpm test`도 `tsc`도 이걸 못 본다** — 그 스크립트들은 어느 쪽에도 걸리지 않기 때문이다. 제품이 초록불인 동안 측정 능력이 사라져 있었다.

마지막 항목이 특히 이 문서의 주제다. 앞의 셋은 "문장이 낡았다"인데, 이건 **낡았는지 확인할 수단 자체가 낡은** 경우다.

패턴이 방법론 ③과 같다 — 우리가 쓴 문장이 근거처럼 읽힌다. 다만 여기서는 시간이 더해진다: **처음 쓸 때 참이었던 문장이 나중에 거짓이 되는데, 아무도 다시 읽지 않는다.** 문서를 근거로 결정을 내리기 전에 그 문서가 마지막으로 측정된 때를 확인해야 한다.
...
여기에 네 가지가 더해졌다. 넷 다 **전제**에 관한 것이고, 측정 없이 넘어갔으면 잘못된 설계가 굳었을 것들이다.
...
**④ "안 쟀다"와 "잴 수 없다"를 구분해야 한다.** 미결 11("대상 폐쇄망에서 WebContainer가 부팅되는가")을 한동안 **가장 결정적인 미측정**으로 적어두고 다른 항목들의 우선순위를 그 뒤로 미뤘다. 그런데 그 환경은 **과거 작업에서 온 시나리오였지 현재 조건이 아니었다** — 즉 미측정이 아니라 **측정 불가**였다. 둘의 차이는 실무적이다: 미측정은 "가서 재라"로 끝나지만, 측정 불가한 항목을 결정 근거로 세우면 **아무도 반증할 수 없는 이유로 다른 작업이 계속 밀린다.** 우선순위 목록에 올릴 때 "이걸 재려면 무엇이 필요한가"를 같이 적으면 걸러진다 — 이 경우엔 "그런 환경에 접근할 것"이었고, 그 순간 최상위가 아니게 된다.

**⑤ 잴 수 있다는 게 재야 할 이유는 아니다.** ④의 거울상이고, ④를 배운 직후에 저질렀다. 미결 목록을 정리하고 나서 다음 작업으로 미결 3(미검증 생태계 형태)을 골랐는데, 이유가 **"살아있는 항목 중 유일하게 이 환경에서 닫을 수 있다"** 였다. 그건 결과로 고른 게 아니라 **측정 가능성으로 고른 것**이다. 닫아도 달라지는 결정이 없었고, 실제로 값이 나오는 두 항목(미결 1·9)은 여전히 실제 기계를 기다리고 있었다.

④가 "이걸 재려면 무엇이 필요한가"를 같이 적으라고 했다면, ⑤는 **"이걸 알면 무엇이 달라지는가"**를 같이 적으라는 것이다. 둘 다 없으면 목록은 잴 수 있는 것 쪽으로 조용히 기운다 — 손이 닿는 일이 중요한 일처럼 보이기 때문이다. 그래서 위 목록의 살아있는 항목은 전부 **언제 답하는지**를 함께 적어뒀다.

넷(①②③④)이 **전제**에 관한 것이라면 ⑤는 **무엇을 먼저 할 것인가**에 관한 것이다. 앞의 넷은 틀린 답을 막고, 이건 맞지만 쓸모없는 답을 막는다.
