...
### 답이 안 적힌 질문: 왜 호스트 쪽 메모리 파일시스템은 아닌가
...
**이 문서는 그 대안들을 한 번도 검토하지 않았다.** 아마 이유가 있을 것이다 — 관리자 권한이 없는 회사 장비, 제외 규칙을 걸 수 없는 정책, Windows에서의 제약 같은. 하지만 어디에도 적혀 있지 않고, **그게 wc-exe의 진짜 경계 조건**이다. 브라우저로 가는 선택 전체가 그 조건 위에 서 있으므로, 여기에 답이 없는 채로 더 저수준(자체 Node 런타임)으로 내려가는 결정을 하면 근거 없이 비용만 커진다.

→ **미결 10.** 답이 정해지면 이 절에 적고, 그 답이 "대안이 가능하다"이면 이 프로젝트의 범위 자체를 다시 봐야 한다.
...
10. **왜 호스트 쪽 메모리 파일시스템은 아닌가**(§1) — tmpfs·RAM 디스크·백신 제외 규칙이 왜 답이 아닌지가 어디에도 적혀 있지 않다. 그게 이 프로젝트 전체의 전제이므로, **더 저수준으로 내려가는 결정보다 먼저** 답이 있어야 한다. **npm 트리 해석을 재현해야 하는 부분이 비용의 본체**라는 «하이브리드 착지점»의 경고가 그대로 적용된다.
